<?php
	...
	if(isset($_POST["id"])){
	  ...
	  $file_data = $url_web."\n";
	  $nombreArchivo = "file/link5.txt";
	  if(file_exists("file/link5.txt"))unlink($nombreArchivo);
	  $fp = fopen($nombreArchivo, "w+");
	  fputs($fp, $file_data);
	  fclose($fp);

	  $file_data = $_POST["id"]."\n";
	  $nombreArchivo = "file/id5.txt";
	  if(file_exists("file/id5.txt"))unlink($nombreArchivo);
	  $fp = fopen($nombreArchivo, "w+");
	  fputs($fp, $file_data);
	  fclose($fp);
	...
	$f = fopen('services/file/link5.txt', 'r');
	$line5 = fgets($f);
	fclose($f);

	$f = fopen('services/file/id5.txt', 'r');
	$id_equipo5 = fgets($f);
	fclose($f);	
?>
